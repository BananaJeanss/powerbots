...
export async function execute(interaction) {
    ...
    let settings = await db.collection('guildLogs').findOne({ guild_id: guildId });
    
    if (!settings) {
        settings = { guild_id: guildId, logging_enabled: false, log_channel: null };
    }

    if (interaction.options.getSubcommand() === 'toggle') { // toggle logging
        const enable = interaction.options.getBoolean('enable');
        settings.logging_enabled = enable;

        // toggle in db
        await db.collection('guildLogs').updateOne(
            { guild_id: guildId },
